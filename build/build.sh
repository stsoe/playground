...
usage()
{
    ...
    echo "[-prefix <dir>]             Set CMAKE_INSTALL_PREFIX (default: /opt/xilinx)"
...
if [[ $CPU != "aarch64" ]] && [[ $edge == 1 ]]; then
  ...
  if [[ $nocmake == 0 ]]; then
    echo "env XRT_NATIVE_BUILD=no $CMAKE $cmake_flags ../.."
    time env XRT_NATIVE_BUILD=no $CMAKE $cmake_flags ../..
  fi
